...
def parse_page(id):
    ...
    for item in content.split("\n"):
        if f"new Book(" in item:
            ...
            pages = map(lambda page: page.replace('"', '""').replace('&nbsp;', ' ').replace('  ', ' '), json5.loads(json_raw).get('pages'))
            lines = '"\t"'.join(pages)
...
if __name__ == '__main__':
    # TODO: use actual items from wowhead (some of them aren't found in metadata loads, so be careful). For now I've just filtered some IDs manually from items DB.
