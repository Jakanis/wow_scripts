...
def save_page(id):
    url = WOWHEAD_URL + f'/item={id}'
    r = requests.get(url)
    with open(f'./books/{id}.html', 'w') as output_file:
        output_file.write(r.text)

def parse_page(id):
    content = None
    with open(f'./books/{id}.html', 'r') as input_file:
...
# save_page(745)
# print(parse_page(745))


# for i in book_ids:
#     save_page(i)

# with open(f'books_res.tsv', 'w') as output_file:
#     for i in book_ids:
...
def parse_to_xml():
    ...
    for item in content.split("\n"):
        els = item.split('\t')
        # print(f"{els[1]}_{els[0]}.xml")
        with open(f'./books_res/{els[1]}_{els[0]}.xml', 'w') as output_file:
            output_file.write('<?xml version="1.0" encoding="utf-8"?>\n')
            output_file.write('<resources>\n')
            for count, item in enumerate(els[2:], start=2):
                line = item.replace('<br \/>', '\n')
                if line:
                    output_file.write(f'<string name="PAGE_{count-1}"><![CDATA[{line}]]></string>\n')
                    lines = lines + 1
            output_file.write('</resources>\n')
    print(lines)
